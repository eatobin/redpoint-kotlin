...
fun main() {
    ...
    val firstState: MyState = myStateJsonStringToMyState(hawksJson)
    tailrec fun loop(nextState: MyState) {
        if (nextState.quit.lowercase(Locale.getDefault()) == "q") {
            println()
            ...
            loop(myStateAskContinue(myStatePrintResults(myStateUpdateAndRunNewYear(nextState))))
        }
    }
